...
export default function RealEstateClient() {
  return (
    <main className="bg-[#05060c] text-white min-h-screen">
      {/* Hero */}
      <section className="pt-40 pb-20 px-6 md:px-12 xl:px-20 bg-[#05060c] border-b border-white/10 gradient-mesh">
        <div className="max-w-[1400px] mx-auto">
          <div className="inline-flex items-center gap-2 mb-8 border border-white/10 px-4 py-2 rounded-full bg-white/[0.04]">
            <MapPinned className="w-4 h-4 text-brand" />
            <span className="text-[10px] font-mono uppercase tracking-widest text-white/50">Industries / Real Estate Wholesaling</span>
          </div>
          <h1 className="font-heading font-extrabold text-[clamp(2.5rem,7vw,6rem)] leading-[0.9] tracking-[-0.04em] mb-8 text-white">
            Real estate wholesaling,<br />
            <span className="text-gradient-brand">systemized.</span>
          </h1>
      ...
      <section className="py-24 px-6 md:px-12 xl:px-20">
        <div className="max-w-[1400px] mx-auto">
          <p className="font-mono text-[10px] tracking-[0.3em] uppercase text-white/40 mb-4">/ The Problem</p>
          <h2 className="font-heading font-bold text-3xl md:text-5xl tracking-tight mb-14 max-w-2xl text-white">
            If this sounds familiar, you&apos;re not alone.
          </h2>

          <div className="grid md:grid-cols-2 gap-5">
            {PAIN_POINTS.map((p, i) => (
              <div key={i} className="card-vibrant rounded-2xl p-8">
                <div className="w-11 h-11 flex items-center justify-center border border-white/10 bg-white/[0.03] rounded-xl mb-5">
                  <p.icon className="w-5 h-5 text-brand" />
                </div>
                <h3 className="font-heading font-bold text-xl mb-2 text-white">{p.title}</h3>
      ...
      <section className="py-24 px-6 md:px-12 xl:px-20 bg-[#05060c] border-t border-white/10">
        <div className="max-w-[1400px] mx-auto grid md:grid-cols-2 gap-16 items-start">
          <div>
            <p className="font-mono text-[10px] tracking-[0.3em] uppercase text-white/40 mb-4">/ What We Build</p>
            <h2 className="font-heading font-bold text-3xl md:text-5xl tracking-tight mb-8 text-white">
            ...
            <Link href="/contact" className="hover-trigger inline-flex items-center gap-3 bg-white text-[#05060c] font-bold text-xs uppercase tracking-widest px-8 py-4 hover:bg-white/90 transition-all duration-300 rounded-full">
              Book a Free Consultation <ArrowRight className="w-4 h-4" />
            </Link>
          </div>

          <div className="card-vibrant rounded-2xl p-8 md:p-10">
            <div className="flex items-center gap-3 mb-8">
              <LayoutDashboard className="w-5 h-5 text-brand" />
              <span className="font-mono text-[10px] uppercase tracking-widest text-white/50">Included in your system</span>
            </div>
      ...
      {/* CTA */}
      <section className="relative py-32 px-6 md:px-12 xl:px-20 bg-[#05060c] border-t border-white/10 text-center overflow-hidden">
        <div className="orb-core absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full pointer-events-none opacity-70" />
        <div className="max-w-[900px] mx-auto relative z-10">
          ...
          <Link href="/contact" className="hover-trigger inline-flex items-center gap-3 bg-white text-[#05060c] font-bold text-xs uppercase tracking-widest px-10 py-5 hover:scale-105 transition-all duration-300 rounded-full shadow-[0_0_40px_rgba(255,255,255,0.15)]">
